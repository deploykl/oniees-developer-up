        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <div class="flex justify-between items-center mb-6">
                    ...
                    @can('Usuarios - Crear')
                        <a href="{{ route('users-add') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-lg transition">
                    ...
                    <table class="min-w-full bg-white border border-gray-200 rounded-lg" id="users-table">
                        ...
                        <tbody id="users-table-body">
                            @foreach ($users as $user)
                                <tr id="user-row-{{ $user->id }}" class="border-t border-gray-100 hover:bg-gray-50 transition">
                                    <td class="px-4 py-3 text-sm">{{ $user->id }}</td>
                                    <td class="px-4 py-3 text-sm">{{ $user->name }} {{ $user->lastname }}</td>
                                    <td class="px-4 py-3 text-sm">{{ $user->email }}</td>
                                    <td class="px-4 py-3">
                                        @if ($user->hasRole('Admin'))
                                            <span class="px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-700">Admin</span>
                                        @elseif($user->hasRole('Supervisor'))
                                            <span class="px-2 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-700">Supervisor</span>
                                        @elseif($user->hasRole('Registrador'))
                                            <span class="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-700">Registrador</span>
                                        @elseif($user->hasRole('Ipress'))
                                            <span class="px-2 py-1 text-xs font-medium rounded-full bg-purple-100 text-purple-700">Ipress</span>
                                        @else
                                            <span class="px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-600">Sin rol</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        @if (isset($user->state_id) && $user->state_id == 2)
                                            <span class="px-2 py-1 text-xs font-medium rounded-full bg-emerald-100 text-emerald-700">Activo</span>
                                        @else
                                            <span class="px-2 py-1 text-xs font-medium rounded-full bg-rose-100 text-rose-700">Inactivo</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3">
        ...
        <div x-show="open"
            ...
            <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
                <div class="p-6">
                    ...
                    <p class="mt-2 text-sm text-center text-gray-500">¿Estás seguro de eliminar este usuario? Esta acción no se puede deshacer.</p>
                </div>
                ...
                <div class="flex gap-3 p-6 pt-0">
                    ...
                    <button type="button"
                            x-on:click="if(userId) { deleteUser(userId); open = false; }"
