...
<head>
    ...
    <title>Repositorio DIEM - Observatorio Nacional</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        body { 
            ...
            font-family: 'Inter', 'Aptos', 'Open Sans', sans-serif;
        }
        ...
        /* Ajustes para el main sin sidebar */
        main {
...
<body>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <div class="flex items-center gap-3 mb-2">
                        <div class="w-10 h-10 bg-gradient-to-br from-[#0E7C9E] to-[#38bdf8] rounded-xl flex items-center justify-center shadow-lg">
                            <i class="fas fa-folder-open text-white text-xl"></i>
                        </div>
                        <h1 class="text-3xl md:text-4xl font-bold tracking-tight bg-gradient-to-r from-[#1E3A5F] to-[#0E7C9E] bg-clip-text text-transparent">
                            Repositorio DIEM - OBSERVATORIO NACIONAL
                        </h1>
                    </div>
                    <p class="text-gray-500 text-base ml-12">Infraestructura, equipamiento y establecimientos de salud</p>
                </div>
        ...
        <div class="mb-6 flex items-center gap-2 text-sm text-gray-500">
            <a href="{{ url('/') }}" class="hover:text-[#0E7C9E] transition-colors flex items-center gap-1">
                <i class="fas fa-home text-xs"></i>
                <span>Inicio</span>
            </a>
            <i class="fas fa-chevron-right text-xs"></i>
            <span class="text-[#0E7C9E] font-medium">Repositorio de recursos</span>
        </div>
                ...
                <div class="sticky top-8 space-y-5">
                    ...
                    <div class="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-100 shadow-lg overflow-hidden">
                        <div class="px-5 pt-5 pb-2 border-b border-gray-100 flex items-center gap-2">
                            <div class="w-6 h-6 rounded-lg bg-gradient-to-br from-[#0E7C9E] to-[#38bdf8] flex items-center justify-center">
                            ...
                            @forelse($categories as $category)
                                <a href="#categoria-{{ $category->id }}" 
                                   class="sidebar-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-gray-700 text-sm transition-all group">
                                    <div class="w-7 h-7 rounded-lg bg-[#0E7C9E]/10 flex items-center justify-center group-hover:bg-[#0E7C9E]/20 transition">
                                        <i class="{{ $category->icon ?? 'fas fa-folder' }} text-[#0E7C9E] text-xs"></i>
                                    </div>
                    ...
                    <div class="bg-gradient-to-br from-[#0E7C9E] to-[#0a5c77] rounded-2xl overflow-hidden shadow-lg">
                    ...
                    <section id="categoria-{{ $category->id }}" class="scroll-mt-24 animate-on-scroll">
                        <div class="mb-6">
                            <div class="flex items-center gap-3 flex-wrap">
                                <div class="w-1.5 h-8 bg-gradient-to-b from-[#0E7C9E] to-[#38bdf8] rounded-full"></div>
                                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-[#0E7C9E]/10 to-[#38bdf8]/10 flex items-center justify-center">
                            ...
                            @if($category->description)
                                <p class="text-gray-500 text-sm mt-2 ml-14">{{ $category->description }}</p>
                            @endif
                        </div>
                        ...
                        @forelse($category->subcategories as $subcategory)
                            <div class="mb-10 ml-3 group">
                                <div class="flex items-center gap-2 mb-3">
                                    <div class="w-1 h-5 bg-[#0E7C9E] rounded-full"></div>
                                    <i class="fas fa-tag text-[#0E7C9E] text-sm"></i>
                                ...
                                @if($subcategory->description)
                                    <p class="text-gray-500 text-sm mb-4 ml-4">{{ $subcategory->description }}</p>
                                @endif
                                    ...
                                    @forelse($subcategory->resources as $resource)
                                        <div data-url="{{ $resource->getResourceUrl() }}"
                                            ...
                                            <div class="flex items-start justify-between">
                                                <div class="w-11 h-11 rounded-xl bg-gradient-to-br from-[#0E7C9E]/10 to-[#38bdf8]/10 flex items-center justify-center text-[#0E7C9E] group-hover:scale-110 transition-transform">
                                                    <i class="{{ $resource->getIcon() }} text-xl"></i>
                                                </div>
                                                <div class="opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <i class="fas fa-arrow-up-right-from-square text-gray-400/40 text-sm"></i>
                                                </div>
                                            </div>
                                            <h4 class="font-semibold text-gray-800 mt-3 text-base line-clamp-1">{{ $resource->title }}</h4>
                                            <p class="text-gray-500 text-sm mt-1 line-clamp-2">{{ Str::limit($resource->description ?? 'Sin descripción', 80) }}</p>
        ...
        <div class="modal-container relative bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col w-[95vw] max-w-[1400px] h-[90vh] max-h-[90vh] animate-scale-in">
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-white/95">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-[#0E7C9E] to-[#38bdf8] flex items-center justify-center">
            ...
            <div class="px-6 py-3 bg-gray-50 text-xs text-gray-500 border-t border-gray-100 flex justify-between items-center">
                <span class="flex items-center gap-2"><i class="fas fa-expand-alt mr-1"></i> Dashboard Interactivo</span>
                <button id="openNewTabBtn" class="text-[#0E7C9E] hover:text-[#0a5c77] transition flex items-center gap-2 font-medium">
