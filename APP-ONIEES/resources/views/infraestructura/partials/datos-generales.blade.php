...
<div class="form-container">
            ...
            <div class="form-grid">
                <div class="form-field"><label class="form-label"><i class="fas fa-qrcode"></i> CÓDIGO IPRESS <span
                    ...
                    <div class="field-with-icon"><input type="text" name="codigo_ipress"
                            class="form-input form-input-readonly" value="{{ $establecimiento->codigo ?? '' }}"
                            readonly><i class="fas fa-lock field-icon"></i></div>
                </div>
                <div class="form-field"><label class="form-label"><i class="fas fa-building"></i> NOMBRE DEL EESS <span
                        ...
                        class="form-input form-input-readonly" value="{{ $establecimiento->nombre_eess ?? '' }}"
                        readonly></div>
                <div class="form-field"><label class="form-label"><i class="fas fa-landmark"></i>
                        INSTITUCIÓN</label><input type="text" name="institucion"
                        class="form-input form-input-readonly" value="{{ $establecimiento->institucion ?? '' }}"
                        readonly></div>
                ...
                <div class="form-field"><label class="form-label"><i class="fas fa-chart-line"></i> NIVEL DE
                        ...
                        <option value="I" {{ ($establecimiento->nivel_atencion ?? '') == 'I' ? 'selected' : '' }}>I
                            Nivel</option>
                        <option value="II"
                            {{ ($establecimiento->nivel_atencion ?? '') == 'II' ? 'selected' : '' }}>II Nivel</option>
                        <option value="III"
                            {{ ($establecimiento->nivel_atencion ?? '') == 'III' ? 'selected' : '' }}>III Nivel
                        </option>
                    </select></div>
                ...
                <div class="form-field"><label class="form-label"><i class="fas fa-cubes"></i> TIPO</label><input
                        type="text" name="tipo" class="form-input"
                        value="{{ $establecimiento->tipo ?? '' }}"></div>
                ...
                <div class="form-field"><label class="form-label"><i class="fas fa-clock"></i> HORARIO</label><input
                        type="text" name="horario" class="form-input"
                        value="{{ $establecimiento->horario ?? '' }}"></div>
            </div>
        </div>
    </div>
        ...
        <div class="section-content" :class="open ? '' : 'hidden'">
            <div class="form-grid">
                ...
                <div class="form-field"><label class="form-label"><i class="fas fa-calendar-week"></i> ÚLTIMA
                        RECATEGORIZACIÓN</label><input type="date" name="ultima_recategorizacion"
                        class="form-input" value="{{ $establecimiento->ultima_recategorizacion ?? '' }}"></div>
    ...
    <div id="sec-seguridad" class="form-section" x-data="sectionCounter('sec-seguridad', 3)" x-init="init()">
        <div class="section-header" @click="toggle()">
            <div class="section-header-left">
                <h2 class="section-title"><i class="fas fa-shield-virus"></i> Índice de Seguridad Hospitalaria <span
                        class="section-badge">Evaluación</span><i class="fas accordion-icon"
                        :class="open ? 'fa-chevron-up' : 'fa-chevron-down'"></i></h2>
            </div>
            <div class="progress-counter" @click.stop>
                <div class="counter-number"><span class="completed" x-text="filled"></span><span
        ...
        <div class="section-content" :class="open ? '' : 'hidden'">
            <div class="form-grid">
                <div class="form-field"><label class="form-label">¿CUENTA CON EL DOCUMENTO?</label>
                    <div class="radio-group"><label class="radio-option"><input type="radio"
                                name="tiene_documento_seguridad" value="1"
                                {{ ($format->seguridad_hospitalaria ?? '') == 'SI' ? 'checked' : '' }}
                                onclick="$('#resultado_seguridad_div').slideDown(200)"><span>SÍ</span></label><label
                            class="radio-option"><input type="radio" name="tiene_documento_seguridad"
                                value="0" {{ ($format->seguridad_hospitalaria ?? '') == 'NO' ? 'checked' : '' }}
                                onclick="$('#resultado_seguridad_div').slideUp(200)"><span>NO</span></label></div>
                </div>
            </div>
            <div id="resultado_seguridad_div"
                style="display: {{ ($format->seguridad_hospitalaria ?? '') == 'SI' ? 'block' : 'none' }}; padding: 0 1.5rem 1.5rem;">
                <div class="form-grid">
                    <div class="form-field"><label class="form-label">RESULTADO</label><select
                            name="resultado_seguridad" class="form-select">
                            <option value="">Seleccione</option>
                            <option value="CATEGORIA A"
                                {{ ($format->seguridad_resultado ?? '') == 'CATEGORIA A' ? 'selected' : '' }}>CATEGORIA
                                A</option>
                            <option value="CATEGORIA B"
                                {{ ($format->seguridad_resultado ?? '') == 'CATEGORIA B' ? 'selected' : '' }}>CATEGORIA
                                B</option>
                            <option value="CATEGORIA C"
                                {{ ($format->seguridad_resultado ?? '') == 'CATEGORIA C' ? 'selected' : '' }}>CATEGORIA
                                C</option>
                        </select></div>
                    <div class="form-field"><label class="form-label">FECHA DE EVALUACIÓN</label><input
                            type="date" name="anio_seguridad" class="form-input"
                            value="{{ $format->seguridad_fecha ?? '' }}"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="sec-patrimonio" class="form-section" x-data="sectionCounter('sec-patrimonio', 3)" x-init="init()">
        <div class="section-header" @click="toggle()">
            <div class="section-header-left">
                <h2 class="section-title"><i class="fas fa-landmark"></i> Patrimonio Cultural <span
                        class="section-badge">Declaratoria</span><i class="fas accordion-icon"
                        :class="open ? 'fa-chevron-up' : 'fa-chevron-down'"></i></h2>
            </div>
            <div class="progress-counter" @click.stop>
                <div class="counter-number"><span class="completed" x-text="filled"></span><span
        ...
        <div class="section-content" :class="open ? '' : 'hidden'">
            <div class="form-grid">
                <div class="form-field"><label class="form-label">¿ES PATRIMONIO CULTURAL?</label>
                    <div class="radio-group"><label class="radio-option"><input type="radio"
                                name="patrimonio_cultural" value="1"
                                {{ ($format->patrimonio_cultural ?? '') == 'SI' ? 'checked' : '' }}
                                onclick="$('#patrimonio_data').slideDown(200)"><span>SÍ</span></label><label
                            class="radio-option"><input type="radio" name="patrimonio_cultural" value="0"
                                {{ ($format->patrimonio_cultural ?? '') == 'NO' ? 'checked' : '' }}
                                onclick="$('#patrimonio_data').slideUp(200)"><span>NO</span></label></div>
                </div>
            </div>
            <div id="patrimonio_data"
                style="display: {{ ($format->patrimonio_cultural ?? '') == 'SI' ? 'block' : 'none' }}; padding: 0 1.5rem 1.5rem;">
                <div class="form-grid">
                    <div class="form-field"><label class="form-label">FECHA DE RECONOCIMIENTO</label><input
                            type="date" name="fecha_patrimonio" class="form-input"
                            value="{{ $format->fecha_emision ?? '' }}"></div>
                    <div class="form-field"><label class="form-label">NÚMERO DE RESOLUCIÓN</label><input
                            type="text" name="num_resolucion_patrimonio" class="form-input"
                            value="{{ $format->numero_documento ?? '' }}" placeholder="Ej: 045-2024"></div>
                </div>
            </div>
        </div>
    </div>
