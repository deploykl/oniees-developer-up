...
use App\Exports\Excel\Establecimiento\FormatoIIICExport;
use App\Exports\Excel\Establecimiento\EquiposExport;
use App\Models\ModulosCompletados;

class FormatIIICController extends Controller
{
    public function __construct(){
        $this->middleware(['can:UPS Critica - Inicio'])->only('index');
    }
    ...
    public function index($codigo = null) {
        try {
            ...
            return view('registro.format_iii_c.index', [
                'format' => $format,
    ...
    private function getOrCreateFormat($user, $establecimiento) {
        $format = FormatIIIC::where('id_establecimiento', '=', $establecimiento->id)->first();
    ...
    private function errorView($alerta, $message) {
        return view('errors.error', [
            'title' => html_entity_decode('UPSS Cr&iacute;tica'),
            'alerta' => $alerta,
            'message' => $message,
        ]);
    }
    ...
    public function search($codigo) {
        ...
        $formats = Auth::user()->tipo_rol == 1 ? FormatIIIC::where('codigo_ipre', '=', $codigo) : FormatIIIC::where('codigo_ipre', '=', $codigo)->whereIn('idregion', $id_regiones);
        $format = null; 
        $nombre_eess = "";
        if ($formats->count() == 0) {
            $format = new FormatIIIC();
    ...
    public function export($idformat, $searchItemOne = "", $searchItemTwo = "") {
        ...
        return (new FormatoIIICExport($idformat, $searchItemOne, $searchItemTwo))->download('REPORTE UPS CRITICA.xlsx');
    }
