...
class FormatIVController extends Controller
{
    public function index($codigo = null) {
        ...
        if (Auth::user()->tipo_rol == 3) {
            ...
            if ($establishment != null) {
                $formats = FormatIV::where('id_establecimiento', '=', $establishment->id);
                if ($formats->count() > 0) {
        ...
        return view('registro.format_iv.index', [ 'format' => $format, 'establishment' => $establishment ]);
    }
    ...
    public function search($codigo) {
        ...
        $formats = Auth::user()->tipo_rol == 1 ? FormatIV::where('codigo_ipre', '=', $codigo) : FormatIV::where('codigo_ipre', '=', $codigo)->where('idregion', '=', Auth::user()->region_id);
        $format = null; 
        $nombre_eess = "";
        if ($formats->count() == 0) {
            $format = new FormatIV();
