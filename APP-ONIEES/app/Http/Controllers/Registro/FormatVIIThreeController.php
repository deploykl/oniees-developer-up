...
use App\Models\FormatVIIThree;
use App\Models\Establishment;
use Illuminate\Support\Facades\Auth;

class FormatVIIThreeController extends Controller
{
    public function save(Request $request) {
        try {
            ...
            $formatDetail = new FormatVIIThree();
            $formatDetail->id_format_vii = $format->id;
            $formatDetail->upss = $request->input('upss');
            $formatDetail->ambiente = $request->input('ambiente');
            $formatDetail->egresos = $request->input('egresos');
            $formatDetail->estancia = $request->input('estancia');
    ...
    public function delete(Request $request) {
        try {
            $formatDetail = FormatVIIThree::find($request->input("format_id"));
            $formatDetail->delete();
            return [
                'status' => 'OK',
    ...
    public function edit($id) {
        $formatDetail = FormatVIIThree::find($id);
        return $formatDetail;
    }
    
    public function updated(Request $request) {
        try {
            $formatDetail = FormatVIIThree::find($request->input('id'));
            $formatDetail->upss = $request->input('upss');
            $formatDetail->ambiente = $request->input('ambiente');
            $formatDetail->egresos = $request->input('egresos');
            $formatDetail->estancia = $request->input('estancia');
