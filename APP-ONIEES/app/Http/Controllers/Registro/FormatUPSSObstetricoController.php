...
use App\Models\CentroObstetrico;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Auth;
use App\Http\Controllers\Controller;
use App\Exports\Excel\CentroObstetricoExport;

class FormatUPSSObstetricoController extends Controller
{
    public function __construct(){
        $this->middleware(['can:UPSS Centro Obstetrico - Inicio'])->only('index');
    }
    ...
    public function index() {
        try {
            ...
            return view('registro.format_upss_obstetrico.index', [ 
    ...
    private function errorView($alerta, $message) {
        return view('errors.error', [
            'title' => 'Centro Obst&eacute;trico',
            'alerta' => $alerta,
            'message' => $message,
        ]);
    }
    ...
    public function edit(Request $request) {
        try {
            $formato = CentroObstetrico::find($request->input('id'));
            if ($formato == null) {
    ...
    public function save(Request $request) {
        try {
            ...
            $cantidad = 0;
            if ($formato->id > 0) {
                $cantidad = CentroObstetrico::where('anio', '=', $formato->anio)
            ...
            $formato->parto_operativas = $request->input('parto_operativas');
            $formato->parto_inoperativas = $request->input('parto_inoperativas');
            $formato->dilatacion_operativas = $request->input('dilatacion_operativas');
            $formato->dilatacion_inoperativas = $request->input('dilatacion_inoperativas');
            $formato->puerperio_operativo = $request->input('puerperio_operativo');
            $formato->puerperio_inoperativo = $request->input('puerperio_inoperativo');
            $formato->save();
    ...
    public function encode(Request $request) {
        try {
            ...
            if ($request->input('anio') != "") {
                $where .= " upss_centro_obstetrico.anio = '".$request->input('anio')."' AND";
            } 
            if ($request->input('mes') != "") {
                $where .= " upss_centro_obstetrico.mes = '".$request->input('mes')."' AND";
            }
            if (Auth::user()->tipo_rol == 3) {
                $where .= " upss_centro_obstetrico.id_establecimiento = '".Auth::user()->idestablecimiento_user."' AND";
            }
            if (strlen($where) > 0) {
                $where = substr($where, 0, -3);
            }
            
            $cantidad = DB::table('upss_centro_obstetrico')
                ->join('establishment', 'upss_centro_obstetrico.id_establecimiento', '=', 'establishment.id')
                ->whereRaw($where)
                ->count();
    ...
    public function reporte($where = "") { 
        ...
        return (new CentroObstetricoExport($where))->download('REPORTE UPSS CENTRO OBSTETRICO.xlsx');
    }
}